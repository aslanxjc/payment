...
class WxPayConf_pub(object):
    ...
    ##微信公众号身份的唯一标识。审核通过后，在微信发送的邮件中查看
    #APPID = "wx553656eb669f4f82"
    ##JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    #APPSECRET = "cb14fdf17889e70e0b672ad993bba77f"
    ##受理商ID，身份标识
    #MCHID = "1309194501"
    ##商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    #KEY = "kAHuCc2g4MINcLRk3o0lxT6J1Z04WuZq"
    ...
    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径
    SSLCERT_PATH = "/root/xyd_release/xydian/common/trans_weixinpay/apiclient_cert.pem"
    SSLKEY_PATH = "/root/xyd_release/xydian/common/trans_weixinpay/apiclient_key.pem"
...
class Common_util_pub(object):
    ...
    def getSign(self, obj):
        ...
        #print String,77777777777777777777777777777777
        #签名步骤三：MD5加密
...
class Transfer_pub(Wxpay_client_pub):
    ...
    def __init__(self, timeout=WxPayConf_pub.CURL_TIMEOUT):
        ...
        #设置curl超时时间
        self.curl_timeout = timeout
        super(Transfer_pub, self).__init__()

    def createXml(self):
        '''
        生成接口请求参数
        '''
        self.parameters["mch_appid"] = WxPayConf_pub.APPID  #公众账号ID
        self.parameters["mchid"] = WxPayConf_pub.MCHID  #商户号
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
        return  self.arrayToXml(self.parameters)
...
def trans_to_openid(ip,partner_trade_no,openid,amount,desc):
    """
    """
    trans = Transfer_pub()
    trans.parameters["spbill_create_ip"] = ip  #终端ip      
    trans.parameters["partner_trade_no"] = partner_trade_no  #商户订单号
    #trans.parameters["openid"] = "oZdoUwnlx6yaCKaS8u15Co2YoZT8"  #openid
    trans.parameters["openid"] = openid  #openid
    trans.parameters["check_name"] = "NO_CHECK"  #openid
    trans.parameters["amount"] = amount  #openid
    trans.parameters["desc"] = desc  #openid
    trans.createXml()
...
if __name__ == "__main__":
    ...
    #par_obj.setParameter('notify_url','http://www.huodongjia.com/')
    #par_obj.setParameter('trade_type','JSAPI')
    #par_obj.createXml()
    #print par_obj.getPrepayId()

    trans = Transfer_pub()
    trans.parameters["spbill_create_ip"] = "127.0.0.1"  #终端ip      
    trans.parameters["partner_trade_no"] = "123456789"  #商户订单号
    #trans.parameters["openid"] = "oZdoUwnlx6yaCKaS8u15Co2YoZT8"  #openid
    trans.parameters["openid"] = "1"  #openid
    trans.parameters["check_name"] = "NO_CHECK"  #openid
    trans.parameters["amount"] = "100"  #openid
    trans.parameters["desc"] = "测试"  #openid
    trans.createXml()
