...
def request_for_create_direct(tn, subject, body, total_fee):
    ...
    try:
        request = urllib2.Request('http://wappaygw.alipay.com/service/rest.htm',urlencode(params))
...
def m_create_direct_pay_by_user(tn, subject, body, total_fee):
    flg,request_token = request_for_create_direct(tn, subject, body, total_fee)
    if 1 == flg:
...
def notify_verify(post):
    # 初级验证--签名
    _,prestr = params_filter(post,False)
    mysign = build_mysign(prestr, settings.ALIPAY_KEY, settings.ALIPAY_SIGN_TYPE)
    log.debug('enter prestr:%s'%prestr)
    log.debug('enter hhh build_mysign:%s'%post)
    log.debug('mobile alipay sign:%s'%post.get('sign'))
    log.debug('mobile alipay mysign%s:'%mysign)
    if mysign != post.get('sign'):
        log.debug('notifyData sign not equal')
        return False
    # 二级验证--查询支付宝服务器此条信息是否有效
    params = {}
    notifyData = post.get('notify_data').encode('utf-8')
    log.debug('notifyData:%s'%notifyData)
    notifyRe = re.compile(r'<notify_id>(.*?)</notify_id>')
    notifyList = notifyRe.findall(notifyData)
    params['partner'] = settings.ALIPAY_PARTNER
    params['notify_id'] = notifyList[0]
    ...
    log.debug('veryfy_result:%s'%veryfy_result)
